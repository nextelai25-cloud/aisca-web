...
import { classroomPassword, classroomStorageKey } from '@/lib/bs360-auth';
...
export default function ClassroomGate({
  ...
  const storageKey = classroomStorageKey(classroom);
  const correct = classroomPassword(classroom);

  useEffect(() => {
    try {
      if (window.localStorage.getItem(storageKey) === correct) setUnlocked(true);
    } catch {
        ...
        <div style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px' }}>
          ...
          >
            ...
            >
              ...
              <p style={{ textTransform: 'uppercase', marginBottom: 6, fontSize: 11, letterSpacing: '0.28em', color: '#7dd3fc', fontWeight: 600 }}>
                Classroom {String(classroom).padStart(2, '0')} — Locked
              </p>
              <p style={{ color: 'rgba(255,255,255,0.42)', fontSize: 13.5, marginBottom: 30, fontWeight: 300 }}>
                Enter this classroom&apos;s password to continue
              </p>
              ...
              >
                ...
                {error && (
                  <p style={{ color: '#f87171', fontSize: 12.5, marginTop: 10 }}>
                    Wrong password for this classroom.
                  </p>
                )}
