...
import {
  ...
  makeSounds, BG, DISPLAY, EASE, screenVariants, AnimatedNumber, type Sounds,
} from '../game-ui'
...
export default function QuizHost() {
  ...
  // Controls fade out when idle, reappear on any input (spec: presenter UI stays secondary)
  useEffect(() => {
    if (!sess) return
  ...
  // Lock rapid double-transitions for a beat (spec 63)
  ...
  if (!sess) {
    return (
      <main style={{ minHeight: '100vh', background: BG, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: EASE.entrance }} style={{ width: '100%', maxWidth: 400, textAlign: 'center' }}>
  ...
  // Group states so the question screen persists across open→closed→reveal (enables card→bar morph)
  const st = view?.state
  const screenKey =
    !view ? 'load'
    : st === 'lobby' ? 'lobby'
  ...
  return (
    <main style={{ minHeight: '100vh', background: BG, color: '#fff', fontFamily: "'Inter', system-ui, sans-serif", display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden', cursor: controlsShown ? 'auto' : 'none' }}>
      {/* ambient glows — persistent, never re-mount between screens */}
      ...
      {/* top bar — fades with the rest of the controls */}
      <motion.div animate={{ opacity: controlsShown ? 1 : 0, y: controlsShown ? 0 : -8 }} transition={{ duration: 0.25, ease: EASE.soft }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 22, fontSize: 16, color: 'rgba(255,255,255,0.7)', fontWeight: 600 }}>
          ...
          <span>👥 <AnimatedNumber value={view?.participantCount ?? 0} duration={300} /></span>
        </div>
      </motion.div>

      {/* main — one persistent shell; only the content screen animates in/out */}
          ...
          <motion.div key={screenKey} variants={screenVariants} initial="initial" animate="animate" exit="exit"
            ...
            {!view ? (
              ...
              <div style={{ width: '100%', textAlign: 'center' }}>
                <motion.h2 initial={{ y: -24, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.45, ease: EASE.entrance }} style={{ fontFamily: DISPLAY, fontSize: 'clamp(2.5rem, 6vw, 4.5rem)', fontWeight: 800, marginBottom: 32, textShadow: '0 8px 40px rgba(139,92,246,0.5)' }}>🏆 Leaderboard</motion.h2>
      ...
      {controlsShown ? (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} transition={{ duration: 0.25, ease: EASE.soft }}
          ...
          {p && (
            <motion.button whileHover={{ y: -2 }} whileTap={{ scale: 0.94 }} onClick={doPrimary} style={{ ...ctrlBtn, background: '#fff', color: '#140a2e', fontWeight: 800, padding: '14px 34px', fontSize: 16 }}>{p.label}</motion.button>
          )}
          {(view?.state === 'podium' || view?.state === 'finished') && (
            <>
    ...
    <div style={{ width: '100%', maxWidth: 1240, display: 'grid', gridTemplateColumns: showQR ? '1fr auto' : '1fr', gap: 56, alignItems: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <div style={{ display: 'inline-flex', alignItems: 'baseline', gap: 8 }}>
          <AnimatedNumber value={view.participantCount} duration={350} style={{ fontFamily: DISPLAY, fontSize: 48, fontWeight: 800, color: '#a78bfa' }} />
      ...
      {showQR && (
        <motion.div initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4, ease: EASE.entrance }} style={{ background: '#fff', padding: 22, borderRadius: 26, boxShadow: '0 24px 90px -24px rgba(0,0,0,0.7)' }}>
...
// ── Question stage (shared for open/closed/reveal so the cards morph into bars) ──
function QuestionStage({ view, reveal }: { view: QuizView; reveal: boolean }) {
  ...
  return (
    <div style={{ width: '100%', maxWidth: 1200, display: 'flex', flexDirection: 'column', gap: 26 }}>
      ...
      <motion.h2 key={q.title} initial={{ y: 20, opacity: 0, filter: 'blur(4px)' }} animate={{ y: 0, opacity: 1, filter: 'blur(0px)' }} transition={{ duration: 0.4, ease: EASE.entrance }}
        style={{ textAlign: 'center', fontFamily: DISPLAY, fontSize: 'clamp(2rem, 4.6vw, 4.2rem)', fontWeight: 800, lineHeight: 1.15, maxWidth: 1050, marginLeft: 'auto', marginRight: 'auto' }}>{q.title}</motion.h2>
      {q.image && (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 18, maxWidth: 1100, margin: '0 auto', width: '100%' }}>
          {q.options.map((opt, i) => (
            <motion.div key={i} layoutId={`opt-${i}`} layout="position"
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '26px 28px', minHeight: 88, borderRadius: 18, background: `${OPTION_COLORS[i % OPTION_COLORS.length]}22`, border: `2px solid ${OPTION_COLORS[i % OPTION_COLORS.length]}77`, fontSize: 26, fontWeight: 600, boxSizing: 'border-box' }}>
